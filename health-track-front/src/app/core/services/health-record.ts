...
export class HealthRecord {
  ...
  createFirstRecord(userId: string, value: CreateFirstRecordRequest) {
    console.log('Enpoint -> ' + this.apiUrl + `/health-records/create/${userId}`)
    return this.http.post(this.apiUrl + `/health-records/create/${userId}`, value, { responseType: 'text' })
  }


  viewRecord(): Observable<HealthRecordResponse[]> {
    return this.http.get<HealthRecordResponse[]>(this.apiUrl + '/health-records/viewRecord')
  }
